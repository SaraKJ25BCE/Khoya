...
database/
├── migrations/
│   ├── 001_create_trades.sql              -- one row per trade/strategy
...
└── seed/
    └── seed_demo_data.sql                 -- one example trade for demo purposes
```
...
- **Context-rich, not just numbers**: `trades` stores `market_regime`,
...
(001 → 004), and run each once. Or use the Supabase CLI:
